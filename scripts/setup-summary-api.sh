...
function triggerScAutoUpdate() {
  const cmd = [
    ...
    'URL="${UPDATE_SCRIPT_URL:-https://raw.githubusercontent.com/harismy/sc1forcr/main/scripts/setup-autoscript-compat.sh}"',
    'ALT_URL="$(printf \'%s\' "$URL" | sed \'s|/main/setup-autoscript-compat\\.sh$|/main/scripts/setup-autoscript-compat.sh|\')"',
    'TMP="$(mktemp /tmp/sc-1forcr-update-XXXXXX.sh)"',
    'if ! curl -fsSL "$URL" -o "$TMP"; then',
    '  if [[ "$ALT_URL" != "$URL" ]] && curl -fsSL "$ALT_URL" -o "$TMP"; then',
    '    URL="$ALT_URL"',
    '  else',
